...
#PLANET_MASS     = "1 Earth Mass Planets"
...
def extract_numbers_from_filename(fname):
    parts = fname.rstrip('.det.lc').split('_')
    try:
        subrun = int(parts[-3])
        field  = int(parts[-2])
        eventid= int(parts[-1])
...
# lc header - the new files have header themselves the first line without # is the header
'''
HEADER_LC = [
...
HEADER_LC2 =["Simulation_time", "measured_relative_flux", "measured_relative_flux_error", "true_relative_flux", "true_relative_flux_error",
...
def process_outfile(out_file, chi2_threshold, obsgroup, num_seasons, folder, fields_file, pdf):
    ...
    write_fancy_summary_page(pdf,
                                 N_total, N_detected,
                                 raw_rate, weighted,
                                 det_fields, field_coords)
